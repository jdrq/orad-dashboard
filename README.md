...
│   ├── historico_progresivo.json    ← Comparación histórica nacional Ene-Jul+ (2022-2025)
│   ├── historico_semestral.json     ← Comparación histórica nacional Ene-Jun (2022-2025)
│   └── rb_hist_sc_progresivo.json   ← Comparación histórica Rubro Sede Central Ene-Jul+ (2022-2025)
├── scripts/
│   ├── descargar_xls_mef.py         ← Fase 2a: descarga automatizada (Playwright) de los 13 XLS diarios
│   ├── actualizar_rb_hist_sc.py     ← Actualiza dev_t1/dev_t2/dev_sem en rb_hist_sc_progresivo.json (Bloque 6)
│   ├── actualizar_hist_gores.py     ← Actualiza historico_progresivo.json — ranking GORES (Bloque 8)
...
└── xls/
    ...
    │                                   scripts/actualizar_hist_gores.py (ranking GORES). SÍ se sube a Git.
    ├── historico_rubro/             ← Manual. Fuente para scripts/actualizar_rb_hist_sc.py (Rubro Sede Central). SÍ se sube a Git.
...
Por año (2022–2025): `1T_RUBRO_{año}.xls`, `2T_RUBRO_{año}.xls`, `JULIO_RUBRO_{año}.xls`. Se procesan con `scripts/actualizar_rb_hist_sc.py`, que actualiza `data/rb_hist_sc_progresivo.json`.
...
> - Ene-Jul = `1T` + `2T` + `Julio`
>
> Tanto `scripts/actualizar_rb_hist_sc.py` como `scripts/actualizar_hist_gores.py` aplican esta suma automáticamente y validan el resultado contra el dato ya existente en el JSON (tolerancia 0.5%) **antes** de sobrescribir — si algún año no pasa la validación, ese año se conserva sin cambios (nunca se escribe un dato dudoso) y verás `⚠️ ... BLOQUEADO` en la consola. No lo ignores: revisa manualmente antes de reintentar.
>
...
- **Cerrado (07/07/2026):** `historico_progresivo.json` (Bloque 8) dejó de armarse manualmente — ahora lo genera `scripts/actualizar_hist_gores.py`, con la misma filosofía de validación cruzada y acumulación progresiva que Rubro. De paso se detectó y corrigió un error metodológico real: los promedios nacionales (`prom_dev_pct`/`prom_cert_pct`) usaban promedio simple en vez de ponderado por PIM.
- **⚠️ DESACTUALIZADO — revisar en sesión de README (ver Sesión 8):** el punto "Próximo hito" de abajo decía que Agosto 2026 estaba pendiente, pero ambos scripts (`actualizar_rb_hist_sc.py` y `actualizar_hist_gores.py`) ya tienen `AGOSTO`/`dev_ago` activos y con datos reales cargados (confirmado 14/08/2026). También revisar el estado real de "Fase 2b" en la línea de arriba, que ya fue implementada según Sesión 8.
- **Próximo hito (cuando cierre Agosto 2026):** extender `PERIODOS` en ambos scripts históricos (`scripts/actualizar_rb_hist_sc.py` y `scripts/actualizar_hist_gores.py`) agregando el mes de Agosto — re-exportar los `.xls` correspondientes con la agrupación correcta y correr los scripts.
